...
var menu = {
	...
	init: function(config) {
		...
		$(config.prev).click(function(event) {
			menu.prev();
		})
		.button({
			icons: {
		...
		$(config.next).click(function(event) {
			menu.next();
		})
		.button({
			icons: {
...
var drop = {
	...
	config: {
		rotation: 15,
		lag: 15, // pixel resistance on the drag
		target: "#drop",
		out: "#wrapper"
	},
	...
	initTarget: function() {
		...
		$target.droppable({
			...
			drop: function(event, ui) {
				...
				if ($id === drop.added) {
					// It's a new magnet
					x = event.pageX - ($drag.width() / 2); 
					y = event.pageY - ($drag.height() / 2); 
